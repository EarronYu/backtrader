...
# ----------------------
# prepare data
t0str, t9str = '2019-06-01', '2021-01-01'
symbol = 'ETHUSDT'
fgCov = False
...
def runstrat(window, bbdevs, kcdevs, bias_pct, volatile_pct):
    ...
    cerebro.addstrategy(BBKCBreak, window=int(window), bbdevs=bbdevs, bias_pct=bias_pct, kcdevs=int(kcdevs),
    ...
    dsharp = quantstats.stats.smart_sharpe(returns, periods=180)
    sortino = quantstats.stats.smart_sortino(returns, periods=180)
...
opt = optunity.maximize(runstrat, num_evals=1, solver_name='particle swarm', window=[1, 200], bbdevs=[0.01, 5.0],
                        kcdevs=[0.01, 5.0], bias_pct=[0.01, 0.20], volatile_pct=[0.01, 0.20])
# long running
...
print('策略参数 window=%s, bbdevs=%s, kcdevs=%s, bias_pct=%s, volatile_pct=%s' % (
optimal_pars['window'], optimal_pars['bbdevs'], optimal_pars['kcdevs'], optimal_pars['bias_pct'],
optimal_pars['volatile_pct']))
print('\n#优化成功')
# 利用最优参数最后回测一次，作图
cerebro = bt.Cerebro()
cerebro.addstrategy(BBKCBreak, window=int(optimal_pars['window']), bbdevs=optimal_pars['bbdevs'],
                    ...
                    volatile_pct=optimal_pars['volatile_pct'])
...
tframes = dict(minutes=bt.TimeFrame.Minutes, days=bt.TimeFrame.Days, weeks=bt.TimeFrame.Weeks,
               months=bt.TimeFrame.Months, years=bt.TimeFrame.Years)
cerebro.resampledata(data, timeframe=tframes['minutes'], compression=10, name='Real')
